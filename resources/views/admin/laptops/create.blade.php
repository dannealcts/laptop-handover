<x-app-layout>
    <div class="py-6 px-6 bg-gray-100 min-h-screen">
        <div class="max-w-4xl mx-auto bg-white shadow rounded-md p-8">
            ...
            <h2 class="text-2xl font-semibold text-gray-800 mb-6">Add New Laptop</h2>
            ...
            <form action="{{ route('admin.laptops.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="specs" id="specs" rows="3"
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('specs') border-red-500 @enderror">{{ old('specs') }}</textarea>
                    @error('specs')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
